# 使用TagBased_TDIDF算法对Delicious2K数据进行推荐
...
class TagBased_TDIDF():
    ...
    def initStat(self):
        ...
        print("user_tags大小 %d, tag_items大小 %d, user_items大小 %d, tag_users大小 %d" % \
              (len(self.user_tags), len(self.tag_items), len(self.user_items), len(self.tag_users)))
    ...
    def recommend(self, user, N):
        ...
        for tag, wut in self.user_tags[user].items():
            mid1 = self.tag_users[tag].values()
            mid2 = list(mid1)
            tag_user_sum = sum(mid2)
            for item, wti in self.tag_items[tag].items():
                if item in tagged_items:
                    continue
                if item not in recommend_items:
                    recommend_items[item] = wut * wti / math.log(1 + tag_user_sum)
                else:
                    recommend_items[item] += wut * wti / math.log(1 + tag_user_sum)
...
if __name__ == '__main__':
    stb = TagBased_TDIDF(r'D:\pythonData\L2_data\user_taggedbookmarks-timestamps.dat')